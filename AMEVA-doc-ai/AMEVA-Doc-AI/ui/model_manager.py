...
class ModelManagerDialog(QDialog):
    ...
    def initUI(self):
        ...
        title.setStyleSheet("font-size: 14pt; font-weight: bold; color: #f39c12; padding-bottom: 10px;")
    ...
    def populate_table(self):
        ...
        for row, model_data in enumerate(self.available_models):
            ...
            is_installed = m_name in self.installed_model_names
            ...
            if m_name in self.pull_workers:
                pbar = QProgressBar()
                pbar.setRange(0, 100)
                pbar.setStyleSheet("QProgressBar { height: 15px; } QProgressBar::chunk { background-color: #3498db; }")
                action_layout.addWidget(pbar)
                self.pull_workers[m_name]['pbar'] = pbar
            elif is_installed:
                btn = QPushButton("삭제")
                btn.setStyleSheet("background-color: #c0392b; padding: 5px;")
                btn.clicked.connect(lambda checked, name=m_name: self.delete_model(name))
                action_layout.addWidget(btn)
            else:
                btn = QPushButton("설치")
                btn.setStyleSheet("background-color: #2980b9; padding: 5px;")
    ...
    def download_finished(self, model_name, success):
        ...
        if success:
            QMessageBox.information(self, "설치 완료", f"{model_name} 완료.")
